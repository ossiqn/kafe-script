...
if (!isset($_SESSION['kullanici']) || $_SESSION['rol'] !== 'barista') { header('Location: login.php'); exit; }
...
<title>Barista | <?= htmlspecialchars($ayarlar['kafe_adi'] ?? 'Lumière') ?></title>
<link href="https://fonts.googleapis.com/css2?family=Playfair+Display:wght@600;700&family=Poppins:wght@300;400;500;600&display=swap" rel="stylesheet">
<style>
:root[data-theme="dark"]{--bg:#080705;--bg2:#0d0b08;--card:#0f0d09;--card2:#131008;--text:#ede8df;--text2:#a09070;--text3:#5a4e3a;--border:#1e1a12;--border2:#2a2418;--gold:#D4AF37;--gold-bg:rgba(212,175,55,0.08);--gold-glow:rgba(212,175,55,0.2);--red:#e05555;--red-bg:rgba(224,85,85,0.1);--green:#4caf82;--green-bg:rgba(76,175,130,0.1);--orange:#e8873a;--orange-bg:rgba(232,135,58,0.1);--blue:#5b9bd5;--blue-bg:rgba(91,155,213,0.1);--sh:0 2px 16px rgba(0,0,0,0.35);--r:13px;--rs:8px;--t:0.2s ease;}
:root[data-theme="light"]{--bg:#f4f1eb;--bg2:#ede9e0;--card:#fff;--card2:#faf8f4;--text:#2a2318;--text2:#6b5e4a;--text3:#a89880;--border:#e8e0d0;--border2:#d4c8b4;--gold:#9a7d3a;--gold-bg:rgba(154,125,58,0.09);--gold-glow:rgba(154,125,58,0.2);--red:#c0392b;--red-bg:rgba(192,57,43,0.08);--green:#2d7a4f;--green-bg:rgba(45,122,79,0.08);--orange:#c07030;--orange-bg:rgba(192,112,48,0.08);--blue:#2563a8;--blue-bg:rgba(37,99,168,0.08);--sh:0 2px 16px rgba(42,35,24,0.07);--r:13px;--rs:8px;--t:0.2s ease;}
...
.tb-role{font-size:0.68em;background:rgba(212,175,55,0.1);border:1px solid var(--border2);padding:3px 10px;border-radius:20px;text-transform:uppercase;letter-spacing:1px;color:var(--gold);}
...
.filter-btn.blue.active{background:var(--blue-bg);border-color:var(--blue);color:var(--blue);}
...
.kolon-hd.blue h3{color:var(--blue);}
...
.kolon-hd.blue .kolon-cnt{background:var(--blue-bg);color:var(--blue);}
...
.kart-masa{font-family:'Playfair Display',serif;font-size:1em;font-weight:700;color:var(--gold);}
...
.kart-adet{font-size:0.75em;color:var(--gold);}
...
.kart-btn.teslim{background:var(--blue-bg);color:var(--blue);border:1px solid var(--blue);}
.kart-btn.teslim:hover{background:var(--blue);color:#fff;}
...
.toast.yeni{border-left:3px solid var(--gold);}
...
<div class="topbar">
    ...
    <span class="tb-role">☕ Barista</span>
    <span class="tb-yeni" id="yeniSayac" style="display:none">0 yeni</span>
...
<div class="main">
    <div class="filter-row">
        <button class="filter-btn red active" onclick="setFilter('bekliyor',this)">Bekliyor</button>
        <button class="filter-btn orange" onclick="setFilter('hazirlaniyor',this)">Hazırlanıyor</button>
        <button class="filter-btn green" onclick="setFilter('hazir',this)">Hazır</button>
        <button class="filter-btn blue" onclick="setFilter('teslim',this)">Teslim</button>
        <button class="filter-btn" onclick="setFilter('hepsi',this)">Hepsi</button>
    </div>

    <div class="board">
        <div class="kolon" id="kolon-bekliyor">
        ...
        <div class="kolon" id="kolon-hazirlaniyor">
        ...
        <div class="kolon" id="kolon-hazir">
        ...
        <div class="kolon" id="kolon-teslim">
            <div class="kolon-hd blue"><h3>Teslim</h3><span class="kolon-cnt" id="cnt-teslim">0</span></div>
            <div class="kolon-body" id="kol-teslim"><div class="bos-durum">Teslim edilen yok</div></div>
        </div>
    </div>
</div>
...
function setFilter(f, btn) {
    ...
    ['bekliyor','hazirlaniyor','hazir','teslim'].forEach(k => {
        document.getElementById('kolon-' + k).style.display = (f === 'hepsi' || f === k) ? '' : 'none';
    });
}
...
async function durumGuncelle(id, yeniDurum) {
    ...
    if (d.durum === 'ok') {
        ...
        toast(d.mesaj || 'Hata oluştu', 'err');
    }
}

async function yukle() {
    try {
        const r = await fetch('ajax.php?islem=siparisleri_getir&hedef=barista');
        const d = await r.json();
        ...
        const gruplar = { bekliyor: [], hazirlaniyor: [], hazir: [], teslim: [] };
        siparisler.forEach(s => { if (gruplar[s.durum]) gruplar[s.durum].push(s); });

        Object.keys(gruplar).forEach(durum => {
            document.getElementById('cnt-' + durum).textContent = gruplar[durum].length;
            renderKolon('kol-' + durum, gruplar[durum], durum);
        });

        const bekSayisi = gruplar.bekliyor.length;
        const sayac = document.getElementById('yeniSayac');
        sayac.textContent = bekSayisi + ' yeni';
        sayac.style.display = bekSayisi > 0 ? '' : 'none';
    } catch(e) {}
}
...
function renderKolon(id, items, durum) {
    ...
    el.innerHTML = items.map(s => {
        ...
        let btn = '';
        if (durum === 'bekliyor')     btn = `<button class="kart-btn hazirla" onclick="durumGuncelle(${s.id},'hazirlaniyor')">Hazırlamaya Başla</button>`;
        if (durum === 'hazirlaniyor') btn = `<button class="kart-btn hazir"   onclick="durumGuncelle(${s.id},'hazir')">Hazır ✓</button>`;
        if (durum === 'hazir')        btn = `<button class="kart-btn teslim"  onclick="durumGuncelle(${s.id},'teslim')">Teslim Et</button>`;
        if (durum === 'teslim')       btn = `<button class="kart-btn" style="opacity:.45;cursor:default;background:var(--blue-bg);color:var(--blue);border:1px solid var(--blue)" disabled>Teslim Edildi ✓</button>`;
...
function toggleTheme() {
    ...
    localStorage.setItem('lm_b_theme', next);
}

window.addEventListener('DOMContentLoaded', () => {
    const saved = localStorage.getItem('lm_b_theme') || 'dark';
